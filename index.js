...
async function starts() {
	...
	onlydev.on('chat-update', async (odc) => {
		try {
			...
			if (budy.match('Bot')){
onlydev.sendMessage(from, `Iya ada apa kak panggil bot ? ketik #menu untuk menggunakan bot 🙂`, audio, { quoted :  freply })
			}
			if (budy.match('Terima Kasih')){
onlydev.sendMessage(from, `Sama Sama Kak 🙂`, audio, { quoted :  freply })
			}
			if (budy.match('Thanks')){
onlydev.sendMessage(from, `Sama Sama Kak 🙂`, audio, { quoted :  freply })
			}
    //function antilink
		...
		setTimeout( () => {
			onlydev.groupRemove(from, [kic]).catch((e)=>{reply(`ERR: ${e}`)})
		}, 5000)
		setTimeout( () => {
			onlydev.updatePresence(from, Presence.composing)
			reply("Waitt...")
		...
		setTimeout( () => {
			onlydev.updatePresence(from, Presence.composing)
			reply("Siapp siap aja dulu jangan tegang :v")
		}, 3000)
		setTimeout( () => {
			onlydev.updatePresence(from, Presence.composing)
			reply("Siapp ?")
		       ...
		       case 'bass':
	             ...
	             case 'toptt':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
					    ...
					    break
					case 'antilink':
					if (isGroupAdmins) return reply('Ini Khsus Admin Ngentot :v')
					if (args.length < 1) return reply('Boo :𝘃')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*Sudah aktif tod* !!!')
						antilink.push(from)
